...
# Run scanner
/usr/local/bin/lw-scanner image evaluate  ${INPUT_IMAGE_NAME} ${INPUT_IMAGE_TAG} --docker-server ${INPUT_DOCKER_REGISTRY} --docker-username ${INPUT_DOCKER_USERNAME} --docker-password ${INPUT_DOCKER_PASSWORD} --build-plan ${GITHUB_REPOSITORY} --build-id ${GITHUB_RUN_ID} --data-directory ${GITHUB_WORKSPACE} ${SCANNER_PARAMETERS}
